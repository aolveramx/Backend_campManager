...
let datesConversion = function(searchObject) {
    ...
    if(searchObject.to) {
        ...
        to = {
            'year':parseInt(moment().add(3,'years').format('YYYY-MM-DD').split('-')[0]),
            'month':parseInt(moment().add(3,'years').format('YYYY-MM-DD').split('-')[1]),
            'day':parseInt(moment().add(3,'years').format('YYYY-MM-DD').split('-')[2])
        }
    }
    return {from,to}
}
